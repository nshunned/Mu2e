//  Optical_Grease_Study_Configuration_5.cpp
//  Created by Ningshun Chen on 06/07/18.

/*
 PE yield study comparing optical grease & regular counters.
 
 Uses runs from Configuration 5:
...
void Optical_Grease_Study_Configuration_5(){
    // counter label 292
    
    // grease-run1291 & default-run1028: beam x = 1000 mm, y = 25 mm
    make_pe_distribution_plot(1291,1028,4,4);
    // grease-run1292 & default-run1009: beam x = 1000 mm, y = 75 mm
    make_pe_distribution_plot(1292,1009,6,6);
    
    // grease-run1294 & default-run1030: beam x = 1500 mm, y = 25 mm
    make_pe_distribution_plot(1294,1030,4,4);
    // grease-run1293 & default-run1007: beam x = 1500 mm, y = 75 mm
    make_pe_distribution_plot(1293,1007,6,6);
    
    // grease-run1295 & default-run1032: beam x = 2000 mm, y = 25 mm
    make_pe_distribution_plot(1295,1032,4,4);
    // grease-run1296 & default-run1005: beam x = 2000 mm, y = 75 mm
    make_pe_distribution_plot(1296,1005,6,6);
...
TTree* get_tree_from_run(int run_num){
    ...
    gSystem->cd("Optical_Grease_Study_Configuration_5");
...
void make_pe_distribution_plot(int run_num_spe, int run_num_def, int sipm_spe, int sipm_def){
    ...
    // sets a legend
    TLegend* legend = new TLegend(0.875, 0.875, 0.5, 0.675, "Most probable values:");
    legend->AddEntry(pe_distr_def, Form("No optical grease: %3.1f PE", mpv_def), "F");
    legend->AddEntry(pe_distr_spe, Form("With optical grease: %3.1f PE", mpv_spe), "F");
...
void print_result() {
    ...
    cout << "MPV Optical Grease / MPV Default" << endl;
    myFile << "MPV Optical Grease / MPV Default" << endl;
